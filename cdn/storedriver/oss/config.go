...
type Config struct {
	...
	BufferGuard unit.Bytes `yaml:"buffer_guard"`
...
func (c *Config) applyDefaults() {
	if c.UploadPartSize == 0 {
		c.UploadPartSize = DefaultPartSize
	}
	if c.DownloadPartSize == 0 {
		c.DownloadPartSize = DefaultPartSize
	}
	if c.UploadConcurrency == 0 {
		c.UploadConcurrency = DefaultConcurrency
	}
	if c.DownloadConcurrency == 0 {
		c.DownloadConcurrency = DefaultConcurrency
	}
	if c.BufferGuard == 0 {
		c.BufferGuard = DefaultBufferGuard
	}
	if c.ListMaxKeys == 0 {
		c.ListMaxKeys = DefaultListMaxKeys
	}
}

const (
	DefaultPartSize        = int64(64 * unit.MB)
	DefaultBufferGuard     = 10 * unit.MB
